...
def register_provider(name: str) -> Callable[[Type[BaseProvider]], Type[BaseProvider]]:
    ...
    def decorator(cls: Type[BaseProvider]) -> Type[BaseProvider]:
        if not issubclass(cls, BaseProvider):
            raise TypeError(
                f"Class {cls.__name__} must inherit from BaseProvider to be registered."
            )
        if name in _provider_registry:
            module_logger.warning(
                f"Provider '{name}' is already registered. Overwriting with {cls.__name__}."
            )
    ...
    for filename in os.listdir(provider_dir):
        if (
            ...
            try:
                ...
                module_logger.debug(
                    f"Successfully imported provider module: {module_path}"
                )
            except ImportError as e:
                # Log clearly but don't halt everything, provider might be optional
                module_logger.warning(
                    f"Could not import provider module {module_path}. Error: {e}"
                )
            except Exception as e:
                module_logger.error(
                    f"Unexpected error importing provider module {module_path}: {e}",
                    exc_info=True,
                )
    ...
    **kwargs: Any,
    ...
    if not provider_class:
        ...
        raise ConfigurationError(
            f"Invalid provider type: '{provider_type}'. Available providers: {available}"
        )
    ...
    try:
        ...
        module_logger.error(
            f"Failed to instantiate provider '{provider_type}': {e}", exc_info=True
        )
        # Re-raise as a ConfigurationError or ProviderError depending on context might be better
        raise ConfigurationError(
            f"Could not create instance of provider '{provider_type}': {e}"
        )
