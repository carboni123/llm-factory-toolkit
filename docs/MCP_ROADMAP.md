...
## v1.0 — parity with OpenAI Agents SDK MCP ✅ complete

**Exit criteria met:** no silent surprise for downstream consumers (one
breaking-change commit documented in `docs/MCP_MIGRATION.md`); persistent
sessions are the default so every real workload benefits without opt-in;
the deprecated context-key dispatch path is gone (one-release grace
period elapsed); `docs/MCP.md` has a single feature matrix covering
tools × resources × prompts × transports × managers for at-a-glance
parity checks.

- [x] **Docs migration guide for users upgrading from v0.x** — new `docs/MCP_MIGRATION.md` walks through both breaking changes (legacy context-key removal, persistent default flip) with before/after snippets, detection recipes (`rg "_mcp_dispatch|_mcp_tool_names"`), rationale for each, and a version-pinning fallback (`llm_factory_toolkit>=0.3,<1.0`). Carries forward the two documented v0.3 scope limitations (approval/telemetry are tool-only, auth retry is tool-only) so upgraders see them in one place.
- [x] **Persistent-session manager is the default (stateless opt-in)** — `LLMClient(persistent_mcp=True)` is now the default. Cold-path callers opt out via `persistent_mcp=False`. `add_mcp_server` lazy-create respects the flag. `docs/MCP.md` session-lifecycle table leads with the persistent row; the stateless `MCPClientManager` is still a public class for explicit `mcp_client=` construction. Six construction-branch unit tests updated (plus one new `persistent_mcp=True` redundancy test to catch future default flips). No behaviour change for anything that passed `persistent_mcp=True` explicitly.
- [x] **Deprecated context-key path removed** — `BaseProvider._resolve_external_dispatcher` no longer reads `_mcp_dispatch` / `_mcp_tool_names` from `tool_execution_context`; the typed `external_dispatcher` kwarg is the only supported path. The one-release grace period landed in `f3d17bb` (v0.1 typed-dispatcher release) and is now harvested. `warnings.warn(...DeprecationWarning...)` branch deleted; the helper takes only the dispatcher now. Docstrings on `ExternalToolDispatcher` and `_dispatch_tool_calls` no longer reference the legacy keys. The `test_legacy_mcp_context_keys_still_dispatch_with_deprecation_warning` test is replaced by `test_legacy_mcp_context_keys_no_longer_dispatch` which verifies the keys are silently ignored, no `DeprecationWarning` fires, and the call falls through to `ToolFactory`.
- [x] **Full MCP feature-matrix in `docs/MCP.md`** — three new matrices at the top of the doc: **MCP primitive × capability** (tools/resources/prompts across discovery, dispatch/read, filters, namespacing, approval, telemetry, auth retry, agentic-loop integration), **transport × capability** (stdio vs streamable-HTTP across session open, bearer-token variants, 401 retry, persistence, runtime mutation), and **session manager × behaviour** (`MCPClientManager` vs `PersistentMCPClientManager`). Known limitations section consolidates the tool-only approval/telemetry/auth-retry caveats so they're visible at a glance.
