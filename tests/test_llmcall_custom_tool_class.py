...
class SecretDataTool:
    ...
    DESCRIPTION: str = "Retrieves secret data based on a provided data ID (Class Implementation)."
    ...
    def __init__(self, config_value: str = "default"):
        ...
        module_logger.info(f"SecretDataTool instance created with config: '{self._config}'")
    def execute(self, data_id: str) -> Dict[str, Any]:
        module_logger.info(f"[SecretDataTool] execute() called with data_id: '{data_id}', config: '{self._config}'")
        if data_id == "access_code_class":
            result = {"secret": self.MOCK_PASSWORD, "retrieved_id": data_id, "config_used": self._config}
        else:
...
async def test_openai_custom_tool_class_call():
    ...
    try:
        ...
        print(f"Tool '{custom_tool_instance.NAME}' registered using its class definition.")
        assert len(tool_factory.get_tool_definitions()) == 1
        assert tool_factory.get_tool_definitions()[0]['function']['name'] == SecretDataTool.NAME
        ...
        print(f"LLMClient initialized with model: {client.provider.model} and Tool Factory containing class tool.")
        ...
        pytest.fail(f"ProviderError during API call: {type(e).__name__}: {e}")
         ...
         pytest.fail(f"Unexpected error during custom tool class test: {type(e).__name__}: {e}")