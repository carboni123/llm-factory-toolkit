"""Unit tests for LiteLLMProvider._extract_core_tool_names helper."""

from __future__ import annotations

from llm_factory_toolkit.provider import LiteLLMProvider


class TestExtractCoreToolNames:
    """Covers empty, populated, and missing-key cases for the helper."""

    def test_none_context_returns_empty_set(self) -> None:
        assert LiteLLMProvider._extract_core_tool_names(None) == set()

    def test_empty_dict_returns_empty_set(self) -> None:
        assert LiteLLMProvider._extract_core_tool_names({}) == set()

    def test_missing_key_returns_empty_set(self) -> None:
        ctx = {"other_key": "value"}
        assert LiteLLMProvider._extract_core_tool_names(ctx) == set()

    def test_empty_core_tools_list_returns_empty_set(self) -> None:
        ctx = {"core_tools": []}
        assert LiteLLMProvider._extract_core_tool_names(ctx) == set()

    def test_populated_core_tools(self) -> None:
        ctx = {"core_tools": ["tool_a", "tool_b", "tool_c"]}
        result = LiteLLMProvider._extract_core_tool_names(ctx)
        assert result == {"tool_a", "tool_b", "tool_c"}

    def test_returns_set_type(self) -> None:
        ctx = {"core_tools": ["x"]}
        result = LiteLLMProvider._extract_core_tool_names(ctx)
        assert isinstance(result, set)

    def test_deduplicates_names(self) -> None:
        ctx = {"core_tools": ["dup", "dup", "other"]}
        result = LiteLLMProvider._extract_core_tool_names(ctx)
        assert result == {"dup", "other"}

    def test_single_tool(self) -> None:
        ctx = {"core_tools": ["only_one"]}
        assert LiteLLMProvider._extract_core_tool_names(ctx) == {"only_one"}
