"""E2E tests for xAI (Grok) provider with real API calls."""
...
from .conftest import SECRET, skip_xai

pytestmark = [pytest.mark.asyncio, pytest.mark.integration, skip_xai]


async def test_simple_generation(xai_test_model: str) -> None:
...
async def test_single_tool_call(xai_test_model: str, tool_factory: ToolFactory) -> None:
...
async def test_multi_tool_calls(xai_test_model: str, tool_factory: ToolFactory) -> None:
    client = LLMClient(model=xai_test_model, tool_factory=tool_factory)
    result = await client.generate(
        input=[
...
async def test_structured_output(xai_test_model: str) -> None:
...
async def test_streaming(xai_test_model: str) -> None:
    client = LLMClient(model=xai_test_model)
    chunks: list[str] = []
...
async def test_usage_metadata(xai_test_model: str) -> None:
    client = LLMClient(model=xai_test_model)
    result = await client.generate(
