"""E2E tests for OpenAI provider with real API calls."""
...
from .conftest import SECRET, skip_openai

pytestmark = [pytest.mark.asyncio, pytest.mark.integration, skip_openai]


async def test_simple_generation(openai_test_model: str) -> None:
...
async def test_structured_output(openai_test_model: str) -> None:
...
async def test_streaming(openai_test_model: str) -> None:
    client = LLMClient(model=openai_test_model)
    chunks: list[str] = []
...
async def test_usage_metadata(openai_test_model: str) -> None:
    client = LLMClient(model=openai_test_model)
    result = await client.generate(
