...
def mock_get_secret_part_1_payload(retrieval_source: str) -> ToolExecutionResult:
    ...
    print(
        f"[Mock Tool 1 Payload] Returning LLM content: {content_llm}, Payload: {payload_data}"
    )
...
MOCK_TOOL_PARAMS_1 = {
    "type": "object",
    "properties": {"retrieval_source": {"type": "string"}},
...
def mock_get_secret_part_2_payload(key_identifier: str) -> ToolExecutionResult:
    ...
    print(
        f"[Mock Tool 2 Payload] Returning LLM content: {content_llm}, Payload: {payload_data}"
    )
    return ToolExecutionResult(
        content=content_llm,
        payload=payload_data,
...
MOCK_TOOL_PARAMS_2 = {
    "type": "object",
    "properties": {"key_identifier": {"type": "string"}},
...
def mock_get_secret_part_3_payload(vault_name: str) -> ToolExecutionResult:
    ...
    print(
        f"[Mock Tool 3 Payload] Returning LLM content: {content_llm}, Payload: {payload_data}"
    )
    return ToolExecutionResult(
        content=content_llm,
        payload=payload_data,
...
MOCK_TOOL_PARAMS_3 = {
    "type": "object",
    "properties": {"vault_name": {"type": "string"}},
    "required": ["vault_name"],
}
...
async def test_openai_deferred_payload_processing(openai_test_model: str) -> None:
    ...
    try:
        ...
        try:
            programmatic_secret = (
                retrieved_parts[1] + retrieved_parts[2] + retrieved_parts[3]
            )
        except KeyError as e:
            pytest.fail(
                f"Missing part number {e} when combining secret. Retrieved parts: {retrieved_parts}"
            )
        ...
        pytest.fail(
            f"Unexpected error during deferred payload test: {type(e).__name__}: {e}"
        )
