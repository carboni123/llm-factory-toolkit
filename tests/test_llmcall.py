...
async def test_openai_simple_call():
    ...
    try:
        ...
        print(f"Received response snippet: {response_content[:100] if response_content else 'None'}...")
        ...
        if "authentication" in str(e).lower():
             pytest.fail(f"OpenAI Provider Authentication Error: {e}. Check if API key is valid and has credit.")
        ...
        pytest.fail(f"An unexpected error occurred during the API call: {type(e).__name__}: {e}")