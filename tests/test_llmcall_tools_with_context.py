...
from llm_factory_toolkit.tools.models import ToolExecutionResult # Tool must return this
...
# --- Test Configuration ---
SYSTEM_PROMPT_CONTEXT_TOOL = "You are a helpful assistant. If asked for a password, use the provided tool."
...
class GetUserPasswordTool:
    ...
    description = "Retrieves the password for a given user. The user_id is handled by the system."
    ...
    def __call__(self, user_id: str, system_identifier: str = "general") -> ToolExecutionResult:
        """
        ...
        """
        print(f"[Tool Instance] '{self.name}' called with INJECTED user_id: '{user_id}', system_identifier: '{system_identifier}'")
...
async def test_openai_tool_call_with_context_injection():
    ...
    try:
        ...
        assert tool_factory.get_tool_definitions()[0]["function"]["parameters"]["properties"].get("user_id") is None, \
        ...
        print(f"LLMClient initialized with model: {client.provider.model} and Tool Factory")
        ...
        print(f"Received final response snippet: {response_content[:150] if response_content else 'None'}...")
        print(f"Received tool payloads: {tool_payloads}")


        # 6. Assertions
        assert response_content is not None, "API call returned None for content"
        assert isinstance(response_content, str), f"Expected string response content, got {type(response_content)}"
        ...
        for p_item in tool_payloads:
            ...
            if p_item["tool_name"] == MOCK_TOOL_NAME_CONTEXT:
                ...
                assert isinstance(actual_payload_data, dict), "Tool's actual payload data should be a dict"
                ...
                print(f"Successfully validated payload for tool '{MOCK_TOOL_NAME_CONTEXT}'")
                break
        
        assert found_correct_payload, f"Did not find the expected payload from tool '{MOCK_TOOL_NAME_CONTEXT}' with the correct password."
